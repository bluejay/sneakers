...
=== Controllers
Sneakers is very easy to use in an existing controller. Simply include it within the class and specify which method to call. The two ways to do this are to either: specify an @action before calling __call__(env) or to include it within the env as 'sneakers.action'. __call__(env) is aliased to call so if you specify the action in a middleware, router, etc. no changes need to be added. If you don't want to use 'sneakers.action' as the key where the action is located, use action_key "myKey". You can also set a default action, which is called if no action is specified by using default_to :myMethod. If not specified, the default action is #run.
  ...
  class Controller
    ...
    def call(env)
      @action = :myMethod # Or you could use: env['sneakers.action'] = :myMethod
      __call__(env) # This will run the action for you.
    end
  end
